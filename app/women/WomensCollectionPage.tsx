  ...
  useEffect(() => {
    async function fetchProducts() {
      try {
        ...
        if (data.success) {
          const women = data.products
            .filter((p: any) => p.category?.toLowerCase() === "women")
            .map((p: any) => ({
              ...p,
              slug: p._id,
              images: [p.image],
              ...
              sizes: [4, 5, 6, 7, 8, 9],
              colors: [{ name: "Default", hex: "#C4956A" }],
              style: "Flat",
    ...
    <>
      ...
      <main style={{ paddingTop: "var(--nav-height)", minHeight: "100vh", backgroundColor: "var(--off-white)" }}>
        ...
        <section style={{ position: "relative", height: "340px", overflow: "hidden", backgroundColor: "var(--near-black)" }}>
          <Image src="/images/womens-collection.png" alt="Women's Kolhapuri Collection" fill style={{ objectFit: "cover", opacity: 0.35 }} sizes="100vw" />
          ...
          <div className="container-kw" style={{ position: "relative", height: "100%", display: "flex", flexDirection: "column", justifyContent: "flex-end", paddingBottom: "48px" }}>
            ...
            <motion.h1 initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="font-display" style={{ fontSize: "clamp(2.2rem, 5vw, 3.8rem)", fontWeight: 700, color: "#fff", lineHeight: 1.1 }}>Women&apos;s Collection</motion.h1>
            <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }} style={{ fontSize: "1rem", color: "rgba(255,255,255,0.6)", marginTop: "12px", maxWidth: "500px" }}>{womensProducts.length} elegantly crafted styles for every occasion</motion.p>
          </div>
        </section>
