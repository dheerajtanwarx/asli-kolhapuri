        ...
        <>
          ...
          >
            ...
            <div style={{ flex: 1, overflowY: "auto", padding: "32px" }}>
              {cart.length === 0 ? (
                <div style={{ textAlign: "center", color: "var(--warm-grey)", marginTop: "40px" }}>
                  <p>Your cart is currently empty.</p>
                  <button onClick={() => setCartOpen(false)} style={{ marginTop: "20px", padding: "12px 24px", backgroundColor: "var(--terracotta)", color: "#fff", border: "none", borderRadius: "8px", fontWeight: 600, cursor: "pointer" }}>Continue Shopping</button>
                </div>
              ) : (
                <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
                  {cart.map((item) => (
                    <div key={item.cartItemId} style={{ display: "flex", gap: "16px" }}>
                      ...
                      <div style={{ flex: 1, display: "flex", flexDirection: "column" }}>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "4px" }}>
                          <Link href={`/product/${item.slug}`} onClick={() => setCartOpen(false)} style={{ fontFamily: "var(--font-heading)", fontSize: "1.05rem", fontWeight: 600, color: "var(--matte-black)", textDecoration: "none" }}>{item.name}</Link>
                        ...
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: "auto" }}>
                          <div style={{ display: "flex", alignItems: "center", border: "1px solid var(--sand-beige)", borderRadius: "6px", overflow: "hidden" }}>
                            <button onClick={() => updateCartQty(item.cartItemId, item.qty - 1)} style={{ width: "28px", height: "28px", border: "none", backgroundColor: "transparent", cursor: "pointer" }}>-</button>
